...
def get_hbond_number(mol):
    bonds = {}
    for a1 in mol:
        if a1.symbol=="Cu":
            for a2 in mol:
                if a2.symbol in ["O","Cu"]:
                    d = mol.get_distance(a1.index,a2.index,mic=True)
                    if d < 4.0:
                        if a1.index in bonds.keys():
                            bonds[a1.index] += 1
                        else:
                            bonds[a1.index] = 1 
                    else:
...
while da.get_number_of_unrelaxed_candidates() > 0:
    ...
    a.info['key_value_pairs']['raw_score'] = get_hbond_number(a)
...
for i in range(n_to_test):
    ...
    dyn.run(fmax=0.05, steps=2)
    a3.info['key_value_pairs']['raw_score'] =  get_hbond_number(a3)
